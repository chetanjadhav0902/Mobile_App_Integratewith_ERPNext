...
//   //         site_type: s.site_type || "",
//   //         completed_date: s.completed_date || "",
//   //         structure: s.type_of_mounting || "",
//   //       });
//   //       setImageUri(s.upload_image || null);
//   //       setElectricBillUri(s.upload_electric_bill || null);

//   //       // Fetch all possible site_type and type_of_mounting options dynamically
//   //       const metaRes = await fetch(`${erpUrl}/api/method/frappe.client.get_meta?doctype=Site Survey`, {
//   //         headers: { Cookie: `sid=${sid}` },
//   //       });
//   //       const metaData = await metaRes.json();
//   //       const fields = metaData.message.fields;

//   //       const siteTypeField = fields.find((f) => f.fieldname === "site_type");
//   //       const structureField = fields.find((f) => f.fieldname === "type_of_mounting");

//   //       setSiteTypeOptions(siteTypeField.options ? siteTypeField.options.split("\n") : []);
//   //       setStructureOptions(structureField.options ? structureField.options.split("\n") : []);
...
// useEffect(() => {
//   const fetchSurveyAndOptions = async () => {
//     try {
...
//       // 2️⃣ Fetch DocType metadata to get all select options
//       const metaRes = await fetch(`${erpUrl}/api/resource/DocType/Site Survey`, {
//         headers: { Cookie: `sid=${sid}` },
//       });
//       const metaData = await metaRes.json();
...
//       const siteTypeField = fields.find(f => f.fieldname === "site_type");
//       const siteOptions =
//         siteTypeField?.options
//           ?.split("\n")
//           .map(opt => opt.replace(/^:/, "").trim()) // remove ERPNext colon if any
//           .filter(Boolean) || [];
//       setSiteTypeOptions(siteOptions);

//       // type_of_mounting options
//       const mountingField = fields.find(f => f.fieldname === "type_of_mounting");
//       const mountingOptions =
//         mountingField?.options
//           ?.split("\n")
//           .map(opt => opt.replace(/^:/, "").trim())
//           .filter(Boolean) || [];
//       setStructureOptions(mountingOptions);
//     } catch (err) {
//       console.log("Fetch survey or options failed:", err);
//     }
//   };

//   if (survey?.name) fetchSurveyAndOptions();
// }, [survey]);
...
//   const pickImage = async (type) => {
//     const options = { mediaType: "photo", quality: 0.8, maxWidth: 1024, maxHeight: 1024 };
//     Alert.alert("Select Option", "Choose image source", [
//       {
//         text: "Camera",
//         onPress: () =>
...
//   const uploadFile = async (fileAsset, fieldname) => {
//     if (!fileAsset || (typeof fileAsset === "string" && fileAsset.startsWith("/files/"))) return fileAsset;
//     if (!fileAsset.uri) return null;

//     const data = new FormData();
//     data.append("file", { uri: fileAsset.uri, type: fileAsset.type || "image/jpeg", name: fileAsset.fileName || `upload_${Date.now()}.jpg` });
//     data.append("doctype", "Site Survey");
//     data.append("docname", survey.name);
//     data.append("fieldname", fieldname);
...
//       if (formData.upload_image && formData.upload_image.uri) uploadedImageUrl = await uploadFile(formData.upload_image, "upload_image");
//       if (formData.upload_electric_bill && formData.upload_electric_bill.uri) uploadedBillUrl = await uploadFile(formData.upload_electric_bill, "upload_electric_bill");
...
//         site_type: formData.site_type,
//         completed_date: formData.completed_date,
//         type_of_mounting: formData.structure,
//         upload_image: uploadedImageUrl,
//         upload_electric_bill: uploadedBillUrl,
//       };
...
//       Alert.alert("Success", "Site survey updated successfully!", [{ text: "OK", onPress: () => navigation.goBack() }]);
//     } catch (error) {
//       Alert.alert("Error", "Failed to update site survey: " + error.message);
//     } finally {
//       setSubmitting(false);
...
//     <View style={styles.fieldContainer}>
//       <Text style={styles.fieldLabel}>{label}</Text>
//       <TouchableOpacity style={styles.imagePicker} onPress={() => pickImage(type)} disabled={submitting} activeOpacity={0.7}>
//         <Ionicons name="camera-outline" size={22} color="#3F51B5" />
//         <Text style={styles.imagePickerText}>{currentUri ? "Change Image" : "Pick Image"}</Text>
//       </TouchableOpacity>
//       {currentUri && (
//         <Image source={{ uri: currentUri.startsWith("/files/") ? `${erpUrl}${currentUri}` : currentUri }} style={styles.previewImage} resizeMode="cover" />
//       )}
//     </View>
//   );
...
//           <Text style={styles.sectionTitle}>Lead Information</Text>
//           <View style={styles.row}>
//             {renderField("Lead", survey?.lead)}
//             {renderField("Lead Name", survey?.lead_name)}
//           </View>
//           <View style={styles.row}>
//             {renderField("Contact Email", survey?.contact_mail)}
//             {renderField("Surveyed By", survey?.surveyed_by)}
//           </View>
//         </View>
...
//               <Text style={styles.fieldLabel}>Status</Text>
//               <View style={styles.pickerWrapper}>
//                 <Picker selectedValue={formData.status} onValueChange={(val) => handleInputChange("status", val)} enabled={!submitting}>
//                   <Picker.Item label="Select Status" value="" />
//                   {statusOptions.map((opt, idx) => <Picker.Item key={idx} label={opt} value={opt} />)}
//                 </Picker>
//               </View>
//             </View>
//             {formData.status === "Completed" && (
//               <View style={[styles.fieldContainer, { flex: 1, marginLeft: 8 }]}>
//                 <Text style={styles.fieldLabel}>Completed Date</Text>
//                 <TouchableOpacity style={styles.input} onPress={() => setShowDatePicker(true)} disabled={submitting}>
//                   <Text>{formData.completed_date ? formData.completed_date : "Select Date"}</Text>
//                 </TouchableOpacity>
//                 {showDatePicker && (
//                   <DateTimePicker
//                     value={formData.completed_date ? new Date(formData.completed_date) : new Date()}
//                     mode="date"
//                     display={Platform.OS === "ios" ? "spinner" : "default"}
//                     onChange={(event, selectedDate) => {
//                       setShowDatePicker(false);
//                       if (selectedDate) handleInputChange("completed_date", selectedDate.toISOString().split("T")[0]);
//                     }}
//                   />
//                 )}
//               </View>
...
//               <Text style={styles.fieldLabel}>Site Type</Text>
//               <View style={styles.pickerWrapper}>
//                 <Picker selectedValue={formData.site_type} onValueChange={(val) => handleInputChange("site_type", val)} enabled={!submitting}>
//                   <Picker.Item label="Select Site Type" value="" />
//                   {siteTypeOptions.map((opt, idx) => <Picker.Item key={idx} label={opt} value={opt} />)}
//                 </Picker>
//               </View>
//             </View>
...
//               <View style={styles.pickerWrapper}>
//                 <Picker selectedValue={formData.structure} onValueChange={(val) => handleInputChange("structure", val)} enabled={!submitting}>
//                   <Picker.Item label="Select Structure" value="" />
//                   {structureOptions.map((opt, idx) => <Picker.Item key={idx} label={opt} value={opt} />)}
//                 </Picker>
//               </View>
//             </View>
//           </View>
...
//         {/* Submit Button */}
//         <TouchableOpacity style={[styles.submitButton, submitting && styles.submitButtonDisabled]} onPress={submitForm} disabled={submitting}>
//           {submitting ? <ActivityIndicator size="small" color="#fff" /> : <Text style={styles.submitButtonText}>Submit Site Survey</Text>}
//         </TouchableOpacity>
//       </ScrollView>
//     </SafeAreaView>
//   );
...
//   container: { flex: 1, backgroundColor: "#F4F6F9" },
//   scrollContainer: { paddingBottom: 40, paddingHorizontal: 10 },
//   surveyIdContainer: { backgroundColor: "#E3F2FD", paddingVertical: 10, marginHorizontal: 16, marginVertical: 12, borderRadius: 10, shadowColor: "#000", shadowOpacity: 0.1, shadowOffset: { width: 0, height: 2 }, elevation: 2 },
//   surveyId: { fontSize: width * 0.045, fontWeight: "bold", color: "#1976D2", textAlign: "center" },
//   section: { backgroundColor: "#fff", marginHorizontal: 16, marginBottom: 12, padding: 16, borderRadius: 12, shadowColor: "#000", shadowOpacity: 0.05, shadowOffset: { width: 0, height: 2 }, elevation: 2 },
//   sectionTitle: { fontSize: width * 0.045, fontWeight: "700", color: "#333", marginBottom: 12, borderBottomWidth: 1, borderBottomColor: "#E0E0E0", paddingBottom: 6 },
//   fieldContainer: { flex: 1, marginBottom: 14 },
//   fieldLabel: { fontSize: 13, fontWeight: "600", color: "#555", marginBottom: 6 },
//   fieldValue: { fontSize: 14, color: "#333", padding: 10, backgroundColor: "#F9FAFB", borderRadius: 8, borderWidth: 1, borderColor: "#E0E0E0" },
//   input: { fontSize: 14, color: "#333", paddingVertical: 10, paddingHorizontal: 12, backgroundColor: "#fff", borderRadius: 8, borderWidth: 1, borderColor: "#3F51B5" },
//   pickerWrapper: { borderWidth: 1, borderColor: "#3F51B5", borderRadius: 8, overflow: "hidden" },
//   imagePicker: { flexDirection: "row", alignItems: "center", padding: 12, backgroundColor: "#F8F9FA", borderRadius: 10, borderWidth: 1, borderColor: "#E0E0E0", borderStyle: "dashed" },
//   imagePickerText: { marginLeft: 10, fontSize: 14, color: "#3F51B5", fontWeight: "500" },
//   previewImage: { width: "100%", height: width * 0.55, borderRadius: 10, marginTop: 10 },
//   submitButton: { flexDirection: "row", alignItems: "center", justifyContent: "center", backgroundColor: "#3F51B5", marginHorizontal: 16, marginVertical: 20, paddingVertical: 16, borderRadius: 12, shadowColor: "#3F51B5", shadowOpacity: 0.3, shadowOffset: { width: 0, height: 3 }, elevation: 4 },
//   submitButtonDisabled: { backgroundColor: "#9FA8DA" },
//   submitButtonText: { color: "#fff", fontSize: 14, fontWeight: "bold", marginLeft: 8 },
//   row: { flexDirection: width < 600 ? "column" : "row", justifyContent: "space-between" },
// });
