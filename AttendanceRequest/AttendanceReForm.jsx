...
const ApplyOnDuty = ({ route }) => {
  ...
  const [selectedReason, setSelectedReason] = useState(''); // ✅ empty string instead of null
  ...
  useEffect(() => {
    const fetchReasons = async () => {
      ...
      try {
        ...
        // Filter out unwanted options and empty strings
        options = options
          .map(opt => opt.trim())
          .filter(opt => opt && opt !== "Work From Home" && opt !== "On Duty");

        setReasonList(options);
        setSelectedReason(''); // ✅ keep as empty string for default
      } catch (error) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      setToDate(new Date());
      setSelectedReason(''); // ✅ reset properly
    } catch (error) {
      let errorMessage = 'You have already applied leave on this date and it is approved.';
...
const styles = StyleSheet.create({
  ...
  header: { fontSize: 24, fontWeight: 'bold', marginBottom: 20, textAlign: 'center', color: '#2c3e50' },
  ...
  halfDayContainer: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' },
  label: { fontSize: 16, fontWeight: '600', marginBottom: 10, color: '#2c3e50' },
  row: { flexDirection: 'row', justifyContent: 'space-between' },
  dateButton: { borderWidth: 1, borderColor: '#ddd', padding: 15, borderRadius: 8, width: '48%', backgroundColor: 'white' },
  dateButtonText: { color: '#333', textAlign: 'center', fontSize: 14 },
  pickerContainer: { borderWidth: 1, borderColor: '#ddd', borderRadius: 8, overflow: 'hidden', backgroundColor: 'white' },
  picker: { height: 50, width: '100%', color: 'black' },
  input: { borderWidth: 1, borderColor: '#ddd', padding: 15, borderRadius: 8, minHeight: 120, backgroundColor: 'white', textAlignVertical: 'top', fontSize: 16, color: '#333' },
  button: { backgroundColor: '#4CAF50', padding: 16, borderRadius: 8, alignItems: 'center', elevation: 2, marginTop: 10 },
  disabledButton: { backgroundColor: '#a5d6a7' },
  buttonText: { color: 'white', fontWeight: 'bold', fontSize: 16 },
  errorBox: { backgroundColor: '#ffebee', padding: 15, borderRadius: 8, marginBottom: 20 },
  errorText: { color: '#c62828', fontSize: 14, textAlign: 'center' },
  loader: { position: 'absolute', top: 0, left: 0, right: 0, bottom: 0, backgroundColor: 'rgba(255,255,255,0.7)', justifyContent: 'center', alignItems: 'center', zIndex: 1000 }
});
